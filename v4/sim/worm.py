...
class IKChain:
    def __init__(self, n_links: int, segment_size: float,
        ...
        (matches worm-sim's original behavior). The straight layout is mainly
        useful for debug/test poses; in normal play the chain converges fast
        from any seed."""
        ...
        if facing is not None:
            ...
            x, y = origin_x, origin_y
            for _ in range(n_links):
                hx = x + random.uniform(-1, 1)
                hy = y + random.uniform(-1, 1)
                tx = hx + random.uniform(-1, 1)
                ty = hy + random.uniform(-1, 1)
                self.links.append(IKSegment(segment_size, hx, hy, tx, ty))
                x, y = tx, ty
